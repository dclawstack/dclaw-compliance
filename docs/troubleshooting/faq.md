...
### How do I update DClaw Compliance?

Update the `version` field in the DClawApp CRD. The operator will perform a rolling update.

```bash
kubectl patch dclawapp compliance --type merge -p '{"spec":{"version":"0.2.0"}}'
```
...
### Can I run DClaw Compliance without Kubernetes?

Yes, for local development:

```bash
cd dclaw-compliance/frontend && npm run dev
cd dclaw-compliance/backend && uvicorn dclaw_compliance.main:app --reload
```
