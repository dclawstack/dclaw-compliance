...
# Check events
kubectl get events -n dclaw-compliance --sort-by='.lastTimestamp'

# Verify database connection
kubectl exec -n dclaw-compliance deployment/dclaw-compliance-backend --   python -c "import asyncio; from sqlalchemy import text; ..."
```
...
2. Check the connection string secret:
   ```bash
   kubectl get secret dclaw-compliance-db-credentials -n dclaw-compliance
   ```
